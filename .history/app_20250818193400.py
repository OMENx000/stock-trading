...
def buy():
    ...
    if request.method == "POST":
        symbol, quantity = request.form.get("symbol").upper(), request.form.get("quantity", 0)
        ...
        shares_owned = db.execute("SELECT * FROM shares_owned WHERE user_id = ? AND symbol = ?", user_info["id"]) # get current holdings
...
def sell():
    ...
    shares_owned = db.execute("SELECT * FROM shares_owned WHERE user_id = ? AND symbol = ?", user_info["id"])
