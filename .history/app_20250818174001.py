...
def buy():
    ...
    if request.method == "POST":
        symbol, quantity = request.form.get("symbol"), request.form.get("quantity", 0)
        if quantity < 1: return apology("Enter Valid quantity") # quantity smaller than 1
        ...
        db.execute("INSERT INTO history(user_id, company_name, company_symbol, action, quantity, price, datetime) VALUES (?, ?, ?, ?, ?, ?, ?)",
                   user_info["id"], info["name"], symbol.upper(), "P", quantity, info["price"], dt)


        # add into users shares
        shares_owned = db.execute("SELECT * FROM shares_owned WHERE user_id = ? AND symbol = ?", user_info["id"], symbol.upper()) # get current holdings
        ...
        if shares_owned: # if already have that type of shares
            db.execute("UPDATE shares_owned SET quantity = ?, total_holdings = ? WHERE user_id = ? AND symbol = ?", shares_owned[0]["quantity"] + quantity,
                       shares_owned[0]["total_holdings"] + (quantity * info["price"]), user_info["id"], symbol.upper())
        else:
...
def sell():
    ...
    if request.method == "POST":
        symbol, quantity = request.form.get("symbol"), int(request.form.get("quantity", 0))
        if quantity < 1: return apology("Enter Valid quantity") # quantity smaller than 1
        ...
        shares_owned = db.execute("SELECT * FROM shares_owned WHERE user_id = ? AND symbol = ?", user_info["id"], symbol)
        ...
        db.execute("INSERT INTO history(user_id, company_name, company_symbol, action, quantity, price, datetime) VALUES (?, ?, ?, ?, ?, ?, ?)",
                   user_info["id"], info["name"], symbol.upper(), "S", quantity, info["price"], dt)
